...
$(OUT): $(OBJS) $(ASRC) mmap.inc
	-rm -f $(OUT)
	zcc +zx -vn -m -zorg=$(ORG) -startup=31 -clib=sdcc_iy $(OBJS) $(ASRC) game_loader.asm -o game -pragma-include:zpragma.inc
	...
	appmake +zx -b game_BANK_06.bin -o game_ay.tap --org 49152 --noloader --noheader
	cat loader.tap mcload.tap mcloader.tap game_scr.tap nirvanap.tap game.tap game_ay.tap > $(OUT)
	grep code_compiler_size game.map

$(OUTC): $(OBJS) $(ASRC) mmap.inc
	-rm -f $(OUTC)
	zcc +zx -vn -zorg=$(ORG) -startup=31 -clib=sdcc_iy $(OBJS) $(ASRC) game_loader.asm -o game -pragma-include:zpragma.inc
	appmake +inject -b game_NIRVANAP.bin -o nirvanap_final.bin -i game_NIRVANA_HOLE.bin --offset 6627
	zx7 -f game_scr.bin
	zx7 -f nirvanap_final.bin
	zx7 -f game_CODE.bin
	zx7 -f game_BANK_06.bin
	...
	echo defc LEN_BANK_06 = $(shell wc -c < game_BANK_06.bin.zx7) >> zx7_game_sizes.asm
	zcc +zx -vn -zorg=$(ORG) -startup=31 -clib=sdcc_iy -Ca-DPCOMPRESS $(OBJS) $(ASRC) zx7_game_sizes.asm game_loader.asm -o game -pragma-include:zpragma.inc
	appmake +zx -b game_MCLOAD.bin -o mcload.tap --blockname mcload --org 16384 --noloader
	appmake +zx -b game_LOADER.bin -o mcloader.tap --org 23296 --noloader --noheader
	...
	appmake +zx -b game_BANK_06.bin.zx7 -o game_ay.tap --org 49152 --noloader --noheader
